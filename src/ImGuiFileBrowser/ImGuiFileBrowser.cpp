...
namespace imgui_addons
{
    ...
    bool ImGuiFileBrowser::renderFileListRegion()
    {
        ...
        size_t num_cols = static_cast<size_t>(std::max<float>(1.0f, std::ceil(static_cast<float>(filtered_dirs.size() + filtered_files.size()) / col_items_limit)));
        ...
        if(num_cols > 64)
        {
            size_t exceed_items_amount = (num_cols - 64) * col_items_limit;
            col_items_limit += static_cast<size_t>(std::ceil(exceed_items_amount/64.0));
            num_cols = static_cast<size_t>(std::max<float>(1.0f, std::ceil(static_cast<float>(filtered_dirs.size() + filtered_files.size()) / col_items_limit)));
        }
        ...
        ImGui::Columns(static_cast<int>(num_cols));
    ...
    void ImGuiFileBrowser::filterFiles(size_t filter_mode)
    {
        filter_dirty = false;
